...
async function yt(url, quality, type, bitrate, server = 'en68') {
    ...
    return {
        url: /<a.+?href="(.+?)"/.exec(json2.result)[1],
        thumb,
        title,
        filesizeF: filesize,
        filesize: KB
...
module.exports = {
     ...
     * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
     */
    yta(url, qualyti , server = 'en68') { return yt(url, '128kbps', 'mp3', '128', server) },
    /**
     * Download YouTube Video as Video via y2mate
     * @param {String} url YouTube Video URL
     * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
     */
    ytv360(url , server = 'en68') { return yt(url, '360p' , 'mp4', '360', server) },
    ytv480(url , server = 'en68') { return yt(url, '480p' , 'mp4', '480', server) },
    ytv720(url , server = 'en68') { return yt(url, '720p' , 'mp4', '720', server) },
